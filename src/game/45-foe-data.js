  /* 메아리의 여울 몹 풀 — 메아리의 여울 조우에서 무작위로 올라온다.
     서 있는 열이 곧 사거리이므로, 그 열에서 손이 닿는 놈만 후보가 된다
     (근접을 후열에 세우면 아무것도 못 하고 서 있게 된다).
     역류의 이랑·잔별의 구렁 풀은 설계만 되어 있고 아직 자리가 없다 — 지금은 모든 층이 이 풀을 쓴다. */
  const FOE_SURFACE = [
    {name:'조수의 신도',       maxHp:26, atk:6, reach:'melee',  role:'skirmisher', icon:ICON_CULTIST},
    {name:'익사체',     maxHp:28, atk:7, reach:'melee',  role:'brute',      icon:ICON_WARDEN},
  ...
  const FOE_ELITES = [
    {name:'감시자',            maxHp:40, atk:7, reach:'melee', role:'skirmisher', icon:ICON_WATCHER},
    /* 웅크린 다음 크게 친다 */
    {name:'쇠사슬 파수장',     maxHp:46, atk:8, reach:'melee', role:'brute',      icon:ICON_CHAINWARD,
     intentMod:{guard_up:3}},
    {name:'첫 번째 잠수복',    maxHp:42, atk:7, reach:'mid',   role:'caster',     icon:ICON_FIRSTSUIT},
    /* 속삭임 빈도 매우 높음 */
    {name:'아홉 눈의 조타수',  maxHp:40, atk:6, reach:'mid',   role:'caster',     icon:ICON_HELMSMAN,
     intentMod:{whisper_rear:2, whisper_random:2}},
    /* 삼킨 종을 울려 전열 전체를 친다 */
    {name:'종을 삼킨 자',      maxHp:48, atk:8, reach:'melee', role:'brute',      icon:ICON_BELLEATER,
     intentMod:{attack_all:3}},
    {name:'조수를 부르는 여사제', maxHp:38, atk:6, reach:'mid', role:'caster',    icon:ICON_PRIESTESS},
    /* 가장 단단하다 — 방어 위주 */
    {name:'강철 아가미',       maxHp:52, atk:9, reach:'melee', role:'warden',     icon:ICON_STEELGILL,
     reactMod:{dodge:0.03, guard:0.38, riposte:0.30, rip:6}},
    /* 약한 놈만 골라 끌어내린다. 원거리로 두면 전열에 세울 수 없어 중거리로 둔다 */
    {name:'익사시키는 자',     maxHp:44, atk:8, reach:'mid',   role:'sniper',     icon:ICON_DROWNER},
  ];
  const FOE_BOSS = {name:'지역의 수문장', maxHp:110, atk:10, reach:'mid', role:'brute', icon:ICON_BOSS, kind:'boss', phase:1};
  ...
  const FOE_ENDLESS = [
    ...
    {name:'심연의 낙하자',    maxHp:56, atk:13, reach:'ranged', role:'skirmisher', icon:assetIcon('assets/minion-abyss-diver.png'),
     intentMod:{double_attack_reach:2}, reactMod:{dodge:0.16, guard:0.05, riposte:0.16, rip:6}},
  ];
  const FOE_ENDLESS_BOSS = {
    name:'심연의 태동', hiddenName:'???', maxHp:220, atk:17,
