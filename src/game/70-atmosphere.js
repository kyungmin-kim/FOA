  ...
  function tierSlug(tier){
    if(tier==='메아리의 여울') return 'surface';
    if(tier==='역류의 이랑') return 'mid';
    return 'abyss';
  }
  function currentTier(){
    if(S.screen==='prologue' || S.screen==='prologueFall') return '잔별의 구렁';
    if(S.screen==='gameover') return '잔별의 구렁';
    ...
    if(S.screen==='escape' || S.screen==='tavern' || S.screen==='result') return '메아리의 여울';
