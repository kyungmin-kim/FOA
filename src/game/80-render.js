  ...
  const BGM_TRACKS = {
    ...
    battle:  {src:'assets/bgm-battle.mp3',  gain:1, seconds:227.462},
    fail:   {src:'assets/bgm-fail.mp3',    gain:1, seconds:242.502},
    lighthouse: {src:'assets/bgm-lighthouse.mp3', gain:1, seconds:194.504},
  };
...
${(ownedMarkers().length || S.relics.length) ? `<div class="topbar-collection ${S.relics.length?'with-relics':''} ${ownedMarkers().length?'with-markers':''}">
  ...
  function heroCardHtml(hero){
    ...
    return `
      <div class="hero-card cls-${hero.cls} ${hero.collapsed?'collapsed':''} ${dead?'dead':''} ${breakingDown?'breakdown':''} ${targetable?'targetable':''} ${tutorialPulse?'tutorial-pulse':''} ${idle?'idle':''} ${marks?'has-marks':''} ${flexGlow?'flex-imprint':''}"
  ...
  function foeCardHtml(en, i){
    ...
    return `
      <div class="foe-card ${dead?'dead':''} ${targetable?'targetable':''} ${tutorialPulse?'tutorial-pulse':''} ${isElite?'elite-foe':''} ${isBoss?'boss-foe':''} ${isAwakened?'boss-awakened':''} ${idle?'idle':''} ${confused?'confused':''}"
